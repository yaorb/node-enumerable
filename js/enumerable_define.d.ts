 ...
 */
export declare type AsyncAction<T = any> = (
 ...
 */
export declare type CancelableFactory<TResult = any> = (
 ...
 */
export declare type ItemMessage<T = any> =
  | string
 ...
 * @template T Type of the items.
 */
export declare type Sequence<T = any> =
  | Iterable<T>
  | ArrayLike<T>
  | Iterator<T>
  | IArguments;
...
export interface IEnumerable<T = any> extends Iterable<T>, Iterator<T> {
  ...
  // TODO:
  findIndexOf(predicate: Predicate<T>, thisArg: any): number;
  ...
  // TODO:
  findLastIndexOf(predicate: Predicate<T>, thisArg: any): number;
  ...
  // TODO:
  skipSame(comparer?: Comparer<T>);

  // TODO: 按比例skip，0..1之间，计算出start，to end；
  skipProportion(proportion: number);
  ...
  // TODO:
  takeSame(comparer?: Comparer<T>);

  // TODO:
  takeProportion(proportion: number);
  ...
  map<U = any>(selector: Selector<T, U>): IEnumerable<U>;
  ...
  // same as aggregate
  reduce<TAccumulate = T, TResult = T>(
    func: (accumulator: TAccumulate, item: T) => TAccumulate,
    seed?: TAccumulate,
    resultSelector?: (accumulator: TAccumulate) => TResult,
  ): TResult;

  reduceRight<TAccumulate = T, TResult = T>(
    func: (accumulator: TAccumulate, item: T) => TAccumulate,
    seed?: TAccumulate,
    resultSelector?: (accumulator: TAccumulate) => TResult,
  ): TResult;
  ...
  // TODO:
  splice(start: number, count: number, ...values: T[]): IEnumerable<T>;

  // TODO:根据条件填充
  fill(item: T, start: number, end: number): IEnumerable<T>;

  // TODO: 先将enumerable --> array, 然后利用array的sort，yield*
  sort(comparer?: Comparer<T>): IEnumerable<T>;
  ...
  //   ): TResult;
  toDictionary();
  // toLookup<TKey extends PropertyKey, U = any>(k)

  // TODO: 二叉树排序算法，前，中，后遍历
  toPreOrder();
  toInOrder();
  toPostOrder();
  ...
  // TODO:按照指定的index pick items
  indices(indices: IEnumerable<number>): IEnumerable<T>;

  // TODO:排列算法，repeatable表明可以重复pick item
  permutation();

  // TODO:组合算法，repeatable表明可以重复pick item
  combination(count: number, repeatable: boolean);

  // TODO:根据指定的数量打包
  chunk(chunk: number, offset: number): IEnumerable<IEnumerable<T>>;
  split();

  // TODO:根据条件将序列split，形成chuck数组
  nearSplit();

  // TODO:数据补全
  leftPad(length: number, item: T): IEnumerable<T>;

  // TODO:数据补全
  rightPad(length: number, item: T): IEnumerable<T>;

  rand();
  random();
  randomOrDefault();
  randomProbability();
  randomIndexProbability();
  randomProbabilityOrDefault();

  // TODO:擦去指定数量的item
  wipe(predicate: Predicate<T>, count?: number): IEnumerable<T>;
  ...
  // TODO: 判断是不是集合的子集
  isSub(other: IEnumerable<T>, comparer: EqualityComparer<T>): boolean;

  // TODO: 判断是不是集合的超集
  isSuper(other: IEnumerable<T>, comparer: EqualityComparer<T>): boolean;
  ...
  // TODO: 根据条件判断出序列中是否有冲突项目
  conflict(comparer?: EqualityComparer<T>): boolean;

  // TODO: 计算序列中的比列值
  proportion(predicate: Predicate<T>): number;

  // TODO: 根据排序找到指定个数的序列
  top(count: number, comparer?: Comparer<T>): IOrderedEnumerable<T>;

  // TODO: 和top类似
  bottom(count: number, comparer?: Comparer<T>): IOrderedEnumerable<T>;
}
/**
 ...
 */
export declare type ZipSelector<T = any, U = T, TResult = any> = (
  x: T,
  y: U,
  index: number,
) => TResult;
