...
class EnumerableBase {
  ...
  aggregate(func, seed, resultSelector) {
    ...
    // }
    const _func = !func ? (acc, item) => acc + item : func;
    if (!resultSelector) {
  ...
  *_chunkInner(size) {
   ...
   * @see concatArray()
   */
  *_cloneInner(count, itemSelector) {
  ...
  *_concatArrayInner(sequences) {
   ...
   * @see defaultIfEmpty()
   */
  *_defaultIfEmptyInner(defaultItems) {
    let hasItems = false;
    for (let item of this) {
      hasItems = true;
      yield item;
   ...
   * @see defaultIfEmpty()
   */
  *_defaultSequenceIfEmptyInner(defaultSequence) {
    let hasItems = false;
    for (let item of this) {
      hasItems = true;
      yield item;
   ...
   * @see distinct()
   */
  *_distinctByInner(selector, comparer) {
    const TEMP = [];
    for (let item of this) {
  ...
  *_exceptInner(second, comparer) {
   ...
   * @see groupBy()
   */
  *_groupByInner(keySelector, keyEqualityComparer) {
    const GROUP_LIST = [];
    for (let item of this) {
  ...
  *_intersperseInner(separators) {
   ...
   * @see intersect()
   */
  *_intersectInner(second, comparer) {
    for (let item of this) {
      for (let secondItem of second) {
        if (comparer(item, secondItem)) {
          yield item;
  ...
  *_prependArrayInner(sequences) {
  ...
  product() {
    return this.aggregate(
      (acc, x) => (Enumerable.IS_EMPTY !== acc ? acc * x : x),
      Enumerable.IS_EMPTY,
    );
  }
  /** @inheritdoc */
...
//# sourceMappingURL=enumerable_1.js.map
